...
#define ConstDefineForChar(myKey,myValue)    static const char * myKey = myValue;

ConstDefineForChar(JKSecurityButtonMaximumClickIntervalKey,\
                     "SecurityButtonMaximumClickIntervalKey")
ConstDefineForChar(JKSecurityButtonQuickTapEnableKey,\
                     "SecurityButtonQuickTapEnableKey")
ConstDefineForChar(JKSecurityButtonLastClickTimeKey, \
                     "SecurityButtonLastClickTimeKey")
...
- (void)setMaximumClickInterval:(NSTimeInterval)maximumClickInterval{
    objc_setAssociatedObject(self, JKSecurityButtonMaximumClickIntervalKey, @(maximumClickInterval), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    // 用OBJC_ASSOCIATION_RETAIN_NONATOMIC是因为存的是NSNumber类型
}

- (NSTimeInterval)maximumClickInterval{
    return [objc_getAssociatedObject(self, JKSecurityButtonMaximumClickIntervalKey) doubleValue];
}


- (void)setLastClickTime:(NSTimeInterval)lastClickTime{
    objc_setAssociatedObject(self, JKSecurityButtonLastClickTimeKey, @(lastClickTime), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSTimeInterval)lastClickTime{
    return [objc_getAssociatedObject(self, JKSecurityButtonLastClickTimeKey) doubleValue];
}
